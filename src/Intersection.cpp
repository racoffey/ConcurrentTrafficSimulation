...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // Check if traffic light is red and if so ...

    
    
    std::cout << "Intersection #" << _id << " is checking traffic light status" << std::endl;
    if (_trafficLight.getCurrentPhase() == TrafficLightPhase::red) {

        // wait until traffic light turns green
        std::cout << "Intersection #" << _id << " is waiting for green." << std::endl;
        //_trafficLight.waitForGreen();
        std::cout << "Intersection #" << _id << " has received green signal." << std::endl;
    }
